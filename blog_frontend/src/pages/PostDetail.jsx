...
const PostDetail = () => {
  ...
  const { data, refetch, isFetching } = useGetOnePostQuery(postId);
  ...
  const shareText = `Check my post "${state.post?.title}"`;
  ...
  const shareLinks = {
    ...
    gmail: `https://mail.google.com/mail/?view=cm&fs=1&tf=cm` +
       `&to=abdymalikbatyrkulov@gmail.com` +
       `&su=${encodeURIComponent("Interesting post to check out")}` +
       `&body=${encodeURIComponent(
         `Hi!\n\n${shareText} at ${shareUrl}\n\nCheers!`
       )}`
  };
  ...
  useEffect(() => {
    if (contentRef.current) {
      const hasOverflow = contentRef.current.scrollHeight > 600;
      setShowReadMoreButton(hasOverflow);
    }
  }, [state?.post?.text]);

  const isClipped = showReadMoreButton && !state.isFullText;
  ...
  const handleToggleSharePost = async () => {
    ...
    const alreadyShared = state.post.shares?.some(u => u._id === userId) ?? false;
    if (alreadyShared) return;

    try {
      const updatedPost = await shareTogglePost({ postId, userId }).unwrap();
      setState(prev => ({ ...prev, post: updatedPost }));
    } catch (error) {
      ...
      <div className="container">
        <div className="single-post__wrapper">
          <div className="single-post__content">
            <div ref={contentRef} className={`content-clipper ${isClipped ? 'clipped' : ''}`}>
          ...
          <div className="single-post__details">
            <div className="cta-counters">
              ...
              <div className="counts" onClick={() => openModal("share")}>
        ...
        >
          ...
          <div className="modal-buttons">
            <Button
              ...
              isYellow
            />
            <Button btnName="No" onClick={() => closeModal()} isYellow/>
          </div>
        </Modal>
        <Modal
          key="share-modal"
          isOpen={activeModal === "share"}
          onRequestClose={() => closeModal("share")}
        ...
        >
          ...
          <div className="share-grid">
            <a href={shareLinks.telegram} target="_blank" rel="noreferrer" className="share-option" onClick={() => handleToggleSharePost()}>
              <TelegramIcon className="tg-icon" />
              <span>Telegram</span>
            </a>
            <a href={shareLinks.whatsapp} target="_blank" rel="noreferrer" className="share-option" onClick={() => handleToggleSharePost()}>
              <WhatsAppIcon className="wa-icon" />
              <span>WhatsApp</span>
            </a>
            <a href={shareLinks.gmail} target="_blank" rel="noreferrer" className="share-option" onClick={() => handleToggleSharePost()}>
